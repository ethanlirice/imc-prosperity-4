"""
Round 3 final trader: online fair-value anchors and option surface trading.

Main change versus earlier variants:
  - Replaced fixed fair values with online estimates derived from the live book.
  - Used a rolling wall-mid median for VFE and HGP anchors.
  - Reused the online VFE anchor for deep-ITM option fair values.

Measured VFE-only tradeoff on Round 3 days 0-2:
  - Static 5250 anchor + cap=15:      +98,963
  - Rolling median W=2000 + cap=15:  +89,846

The rolling-anchor version gave up some in-sample PnL for portability: it does
not depend on a fixed simulator equilibrium and can follow mean shifts between
rounds. The estimator uses a 2000-tick median window with a 200-tick warmup.
"""
