...
class Trader:
    ...
    def trade_core_option(self, sym, depth, pos, out):
        ...
        dynamic_edge = max(8.0, spread * 1.2)
        lim = POS_LIMIT[sym]
        bc = lim - pos; sc = lim + pos
        if a <= fair - dynamic_edge:
            ...
            if q > 0:
                ...
                sc -= q
        # v14g: passive make inside L1 (spread >= 2 only).
        if spread >= 2:
            make_edge = max(2, int(spread * 0.5))
            mb = int(round(fair - make_edge))
            ma = int(round(fair + make_edge))
            if mb <= b: mb = b + 1
            if ma >= a: ma = a - 1
            if mb < ma:
                make_size = 10
                if bc > 0:
                    out.setdefault(sym, []).append(Order(sym, mb, min(make_size, bc)))
                if sc > 0:
                    out.setdefault(sym, []).append(Order(sym, ma, -min(make_size, sc)))
