...
# TTE clock: backtester data has per-day timestamp resets (each day runs 0..999900).
# We persist day_index in traderData and detect day rollover by ts going down.
# Historical days run as TTE = (8 - day_index - ts/1e6) / 365 across days 0/1/2,
# matching wiki: TTE=8d at start of D0, 7d at start of D1, 6d at start of D2.
TTE_DAYS_AT_FIRST = 8.0
...
def _tte_years(day_index, timestamp):
    elapsed = day_index + timestamp / TICKS_PER_DAY
    return (TTE_DAYS_AT_FIRST - elapsed) / TTE_DAYS_PER_YEAR
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Day-rollover detection: backtester resets timestamp to 0 each day.
        last_ts = td.get("last_ts", -1)
        day_index = td.get("day_index", 0)
        if last_ts >= 0 and state.timestamp < last_ts:
            day_index += 1
        td["day_index"] = day_index
    ...
    def trade_vev_5500(self, state, td):
        ...
        T = _tte_years(td.get("day_index", 0), state.timestamp)
        if T <= 1e-6:
            return []
        ...
        dev_from_rolling = wall_mid - rolling_fair
        ...
        # OPEN: voucher rich vs rolling fair → sell into best_bid
        if dev_from_rolling > THR_OPEN_5500 and sell_cap > 0:
        ...
        # OPEN: voucher cheap vs rolling fair → buy into best_ask
        if dev_from_rolling < -THR_OPEN_5500 and buy_cap > 0:
        ...
        # CLOSE: unwind when crossed back; only close if we did NOT just open this tick
        if sell_done == 0 and position > 0 and dev_from_rolling >= THR_CLOSE_5500:
            avail = vd.buy_orders[best_bid]
            qty = min(avail, position)
            if qty > 0:
                orders.append(Order(VEV_5500, best_bid, -qty))
        elif buy_done == 0 and position < 0 and dev_from_rolling <= -THR_CLOSE_5500:
            avail = -vd.sell_orders[best_ask]
            qty = min(avail, -position)
            if qty > 0:
                orders.append(Order(VEV_5500, best_ask, qty))
