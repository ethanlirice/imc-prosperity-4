...
CORE_OPTION_STRIKES = [4000, 4500, 5000, 5100, 5200, 5300]
...
class Trader:
    ...
    # ── Core option take logic (unchanged from v4) ──
    ...
    def run(self, state):
        ...
        # Options: Core inner-strike takes + smile fit on outer strikes
        ...
        # v14d addition: free-option play on far-OTM strikes pinned at bid 0 / ask 1.
        # End-of-round liquidation is at hidden fair; for these strikes the fair is ~0,
        # so accumulating long at price 0 is cost-zero, downside-zero.
        for K in (6000, 6500):
            sym = f'VEV_{K}'
            if sym not in state.order_depths: continue
            d = state.order_depths[sym]
            ...
            bc = lim - pos
            if bc <= 0: continue
            # Take any ask offered at 0 (free) up to limit.
            if d.sell_orders:
                a = min(d.sell_orders.keys())
                if a <= 0:
                    avail = -d.sell_orders[a]
                    q = min(avail, bc)
                    if q > 0:
                        result.setdefault(sym, []).append(Order(sym, a, q))
                        bc -= q
            # Post a passive bid at 0 with the remaining capacity.
            if bc > 0:
                result.setdefault(sym, []).append(Order(sym, 0, bc))
