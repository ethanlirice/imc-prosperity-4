"""
Round 2 trader v10_final — v8_final plus three additive refinements.

- MAF bid shaded to 511.
- OSMIUM: rolling counterparty-toxicity can widen quotes by 2 ticks briefly.
- PEPPER: one final-ticks unwind-at-fair probe, capped to visible liquidity.
"""
...
PEPPER_UNWIND_POS = 76
PEPPER_UNWIND_TS = 999500
PEPPER_UNWIND_BUFFER = 40
...
class Trader:
    ...
    def _update_toxicity(self, state, td, mid):
        tox = td.setdefault("tox", {})
        keep = []
        for ts0, px, side, cp in td.get("tox_p", []):
            if state.timestamp - ts0 >= TOX_LAG:
                rec = tox.setdefault(cp, [0.0, 0]); rec[0] += (mid - px) if side > 0 else (px - mid); rec[1] += 1
            else:
                keep.append([ts0, px, side, cp])
        last_seen = td.get("tox_last", -1); fresh = set()
        for t in state.own_trades.get(OSMIUM, []):
            ...
            if cp and cp != "SUBMISSION":
                keep.append([t.timestamp, t.price, 1 if t.buyer == "SUBMISSION" else -1, cp]); fresh.add(cp)
            if t.timestamp > last_seen:
                last_seen = t.timestamp
        td["tox_p"] = keep; td["tox_last"] = last_seen
        toxic = {cp for cp, (s, n) in tox.items() if n >= TOX_MIN_COUNT and s / n < TOX_THRESHOLD}
        if fresh & toxic:
            td["tox_until"] = state.timestamp + TOX_WIDEN_TICKS
        return TOX_WIDEN if state.timestamp < td.get("tox_until", -1) else 0
    ...
    def trade_osmium(self, state, td):
        ...
        cur_mid = (max(depth.buy_orders.keys()) + min(depth.sell_orders.keys())) / 2 if has_bids and has_asks else wall_mid
    ...
    def trade_pepper(self, state, trader_data):
        ...
        local_ts = state.timestamp % 1000000
        if not s.get("unwound") and abs(position) >= PEPPER_UNWIND_POS and peak is not None and peak - mid <= PEPPER_UNWIND_BUFFER and local_ts >= PEPPER_UNWIND_TS:
            if position > 0 and best_bid is not None:
                fair = int(mid)
                visible = sum(v for p, v in depth.buy_orders.items() if p >= fair)
                take = min(position, visible)
                if take > 0:
                    orders.append(Order(PEPPER, fair, -take))
                    s["unwound"] = True
                    return orders
            if position < 0 and best_ask is not None:
                fair = int(mid + 0.999999)
                visible = sum(-v for p, v in depth.sell_orders.items() if p <= fair)
                take = min(-position, visible)
                if take > 0:
                    orders.append(Order(PEPPER, fair, take))
                    s["unwound"] = True
