# ============================================================================
# merged.py — v314159 + combined Mark signal layers (does NOT replace v314159)
# ============================================================================
#
# What was merged (Round 4 local backtests, merged days 1–3):
#   1) Baseline stack: identical to strategies/round4/v314159.py (OU/IV/ITM/M22/HGP).
#   2) Mark 14 on HYDROGEL_PACK: after Mark 14 buys HGP, suppress OUR HGP sells for
#      SIGNAL_WINDOW (from exp_mark14_v1_suppress_sell_after_hgp_buy.py). Prior run:
#      none ~+3.5k vs v314159; worse ~+3.1k.
#   3) Mark 67/M49 VFE flash schedule: when M67/M49 signal is active, first
#      MERGED_VFE_FLASH_MS ticks use passive bid edge -1 (join-then-flash style),
#      then revert to PASSIVE_VFE_BID_EDGE (from v314159_mark67_v15). Prior batch:
#      v15 matched v314159 on none; worse slightly higher (~+90).
#
# NOT merged (hurt or redundant vs sweep table in DATA.md):
#   - Shorter SIGNAL_WINDOW alone (1000/500/200).
#   - Heavy “offensive” take/passive tightening from mark67_v3–v8 (large none drawdowns).
#   - Mark 14 aggressive micro-copy at ask (hurts).
#
# Assumption review: historical CSV trade-price edges are not executable post-ID;
# this file uses only risk/off (suppress sells) and passive-edge schedules, no copy.
# ============================================================================
...
# merged.py — Mark 14 HGP + VFE flash (see header)
M14_BUYER = "Mark 14"
M14_HGP_WINDOW = SIGNAL_WINDOW
M14_DEBOUNCE_GAP_TS = 5000  # sparse M14 HGP buys vs raw burst stream
MERGED_VFE_FLASH_MS = 300
MERGED_VFE_FLASH_BID_EDGE = -1.0
...
class Trader:
    ...
    def update_m14_hgp_buy_signal(self, state, ss):
        """Debounced Mark 14 HGP buys — one pulse per M14_DEBOUNCE_GAP_TS."""
        last_acc = ss.get("m14_db_last_acc_ts", -10**9)
        pulse_ts = ss.get("m14_hgp_buy_ts", -10**9)
        for tr in sorted(state.market_trades.get(HYDROGEL, []), key=lambda x: x.timestamp):
            if tr.buyer != M14_BUYER:
                continue
            ts = int(tr.timestamp)
            if ts - last_acc >= M14_DEBOUNCE_GAP_TS:
                last_acc = ts
                pulse_ts = ts
        ss["m14_db_last_acc_ts"] = last_acc
        ss["m14_hgp_buy_ts"] = pulse_ts
        return ss
    ...
    def run(self, state):
        ...
        ss = self.update_m14_hgp_buy_signal(state, ss)
        ...
        m14_ts = ss.get("m14_hgp_buy_ts", -10**9)
        m14_hgp_suppress = (state.timestamp - m14_ts) <= M14_HGP_WINDOW and m14_ts >= 0

        vfe_bid_pe = PASSIVE_VFE_BID_EDGE
        if signal_active:
            dt = state.timestamp - last_ts
            if dt <= MERGED_VFE_FLASH_MS:
                vfe_bid_pe = MERGED_VFE_FLASH_BID_EDGE

        # --- VVF (M67/M49 sell suppression + flash passive bid schedule) ---
        if UNDERLYING in state.order_depths:
            ...
            orders = self.trade_ou_split(
                ...
                vfe_bid_pe, PASSIVE_VFE_ASK_EDGE,
                state.timestamp, fs,
        ...
        # --- Hydrogel (V11 micro-buy; Mark 14 buy -> temporary sell suppression) ---
        if HYDROGEL in state.order_depths:
            ...
            orders = self.trade_ou_split(
                ...
                suppress_sell=m14_hgp_suppress,
            )
            if orders:
