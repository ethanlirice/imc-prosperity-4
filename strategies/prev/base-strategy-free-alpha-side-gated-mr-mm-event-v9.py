...
# Primary R5 backtest: pending
# Candidate v9: v8 plus timestamp-position event schedule overlay.
LIMIT = 10
...
EVENT_TARGETS = {
    0: {"PEBBLES_XL": 10},
    800: {"PEBBLES_XL": 0},
    900: {"MICROCHIP_SQUARE": 10, "PEBBLES_M": 10},
    2500: {
...
def event_targets_for_tick(tick: int) -> Dict[str, int]:
    active: Dict[str, int] = {}
    for event_tick in sorted(EVENT_TARGETS.keys()):
        if tick >= event_tick:
            active = dict(EVENT_TARGETS[event_tick])
        else:
            break
    return active
class Trader:
        ...
        if trader_data:
            try:
                ...
                if isinstance(data, dict):
                    data.setdefault("tick", 0); data.setdefault("res", {}); data.setdefault("mr", {}); return data
            except Exception:
                pass
        return {"tick": 0, "res": {}, "mr": {}}
    ...
    def target_position_order(self, state: TradingState, product: str, target_position: int) -> Optional[Order]:
        position = int(state.position.get(product, 0))
    ...
    def run(self, state: TradingState):
        ...
        event_targets = event_targets_for_tick(tick)
        event_owned = set(event_targets.keys())
        for product, target_position in event_targets.items():
            order = self.target_position_order(state, product, int(target_position))
            if order is not None and order.quantity != 0:
                orders[product] = [order]
        for product in FREE_ALPHA_PRODUCTS:
            if product in event_owned:
                continue
        ...
        for spec in SPECS:
            target = str(spec["target"])
            if target in PASSIVE_TARGET_OVERRIDES or target in FREE_ALPHA_PRODUCTS or target in event_owned:
                continue
        ...
        for product, depth in state.order_depths.items():
            if product in event_owned or product in FREE_ALPHA_PRODUCTS or product not in MM_PRODUCTS or product in ACTIVE_RESIDUAL_TARGETS or product in orders:
                continue
            bid, ask = best_bid_ask(depth)
