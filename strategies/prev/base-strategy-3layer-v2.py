...
# Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +466,166
# Rejected: qty=5 pair-cross version tested at +480,922; current qty=2 version regressed.
# Do not promote over base-strategy.py (+534,757).
...
PAIR_SPECS = [
    {
        ...
        "mean": 347.52773333333334,
        "sigma": 200.08913367345951,
        "entry_z": 1.5,
        "exit_z": 0.5,
        "hold": 1200,
        "qty": 2,
    ...
    {
        "a": "OXYGEN_SHAKE_CHOCOLATE",
        "b": "OXYGEN_SHAKE_GARLIC",
        "mean": -2368.7606333333333,
        "sigma": 729.954675798618,
        "entry_z": 1.5,
        "exit_z": 0.5,
        "hold": 1600,
        "qty": 2,
...
for spec in PAIR_SPECS:
    PAIR_PRODUCTS.add(str(spec["a"]))
    PAIR_PRODUCTS.add(str(spec["b"]))
...
class Trader:
    ...
    def pair_spread(self, state: TradingState, spec: Dict[str, Any]) -> Optional[float]:
        a = str(spec["a"])
        b = str(spec["b"])
    ...
    def pair_entry_orders(
        ...
        if depth_a is None or depth_b is None:
            return []
        orders: List[Order] = []
        result_a = active_price_for_qty(depth_a, direction, qty)
        result_b = active_price_for_qty(depth_b, -direction, qty)
        if result_a is not None:
            price_a, qty_a = result_a
            orders.append(Order(a, price_a, qty_a if direction > 0 else -qty_a))
        if result_b is not None:
            price_b, qty_b = result_b
            orders.append(Order(b, price_b, -qty_b if direction > 0 else qty_b))
        return orders

    def pair_close_orders(self, state: TradingState, spec: Dict[str, Any], direction: int) -> List[Order]:
        a = str(spec["a"])
        b = str(spec["b"])
        qty = int(spec["qty"])
        depth_a = state.order_depths.get(a)
        ...
        if depth_a is None or depth_b is None:
            return []
        pos_a = int(state.position.get(a, 0))
        pos_b = int(state.position.get(b, 0))
        orders: List[Order] = []
        if pos_a != 0:
            result_a = active_price_for_qty(depth_a, -1 if pos_a > 0 else 1, abs(pos_a))
            if result_a is not None:
                price_a, qty_a = result_a
                orders.append(Order(a, price_a, qty_a if pos_a < 0 else -qty_a))
        if pos_b != 0:
            result_b = active_price_for_qty(depth_b, -1 if pos_b > 0 else 1, abs(pos_b))
            if result_b is not None:
                price_b, qty_b = result_b
                orders.append(Order(b, price_b, qty_b if pos_b < 0 else -qty_b))
        return orders
    ...
    def run(self, state: TradingState):
        ...
        pair_sleeves = memory.setdefault("pairs", {})
        ...
        for spec in PAIR_SPECS:
            ...
            b = str(spec["b"])
            key = f"{a}|{b}"
            sleeve = pair_sleeves.setdefault(key, {})
            pos_a = int(state.position.get(a, 0))
            pos_b = int(state.position.get(b, 0))
            if pos_a == 0 and pos_b == 0:
                sleeve.clear()

            spread = self.pair_spread(state, spec)
            if spread is None:
                continue
            z = (spread - float(spec["mean"])) / float(spec["sigma"])
            target_orders: List[Order] = []
            entry_tick = int(sleeve.get("entry_tick", tick))
            in_pair = pos_a != 0 or pos_b != 0
            should_exit = False
            if in_pair:
                timed_out = tick - entry_tick >= int(spec["hold"])
                if abs(z) <= float(spec["exit_z"]) or timed_out:
                    should_exit = True

            if should_exit:
                target_orders.extend(self.pair_close_orders(state, spec, int(sleeve.get("dir", 0))))
                sleeve.clear()
            elif not in_pair:
                direction = 0
                if z <= -float(spec["entry_z"]):
                    ...
                    direction = -1
                if direction != 0:
                    target_orders.extend(self.pair_entry_orders(state, spec, direction))
                    if target_orders:
                        sleeve["dir"] = direction
                        sleeve["entry_tick"] = tick
                        sleeve["entry_spread"] = round(float(spread), 4)

            if target_orders:
                orders[a] = [o for o in target_orders if o.symbol == a]
                orders[b] = [o for o in target_orders if o.symbol == b]
                if not orders[a]:
                    orders.pop(a, None)
                if not orders[b]:
                    orders.pop(b, None)
        ...
        for product, depth in state.order_depths.items():
            if product not in MM_PRODUCTS or product in PAIR_PRODUCTS or product in RESIDUAL_TARGETS or product in orders:
                continue
            bid, ask = best_bid_ask(depth)
