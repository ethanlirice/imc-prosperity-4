...
# Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +623,546
# Days: D2 +206,398 / D3 +181,641 / D4 +235,507
# Experiment: touch-MM baseline, but products where pure passive touch beat the
# residual sleeve are owned by passive MM instead of active residual trading.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for spec in SPECS:
            target = str(spec["target"])
            if target in PASSIVE_TARGET_OVERRIDES:
                continue
        ...
        for product, depth in state.order_depths.items():
            if product not in MM_PRODUCTS or product in ACTIVE_RESIDUAL_TARGETS or product in orders:
                continue
            bid, ask = best_bid_ask(depth)
