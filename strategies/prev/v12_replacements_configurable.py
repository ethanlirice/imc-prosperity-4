...
# Configurable replacement tester for v12.
# Based on v11 root.
LIMIT = 10
...
REANCHOR_PATHS = {
    ...
    # Phase D Replacements
    "MICROCHIP_SQUARE": {"deltas": [0.0, 736.7014, 1180.1451, 1090.3584, 1146.5399], "edge": 300.0},
    "UV_VISOR_RED": {"deltas": [0.0, 238.4507, 316.3363, 83.0979, 261.0775], "edge": 120.0},
    "SLEEP_POD_LAMB_WOOL": {"deltas": [0.0, 322.4222, 403.3659, 166.5093, 331.7787], "edge": 20.0},
    "SNACKPACK_RASPBERRY": {"deltas": [0.0, -124.4750, -1.5672, -27.9195, 69.6999], "edge": 120.0},
    "UV_VISOR_MAGENTA": {"deltas": [0.0, 44.0339, 31.0452, 316.0422, 382.3820], "edge": 40.0},
}
_ra_env = os.environ.get("RA_PRODUCTS", "").strip()
REANCHOR_PRODUCTS = {
    "ROBOT_DISHES",
    "PEBBLES_L",
    "OXYGEN_SHAKE_MINT",
...
if _ra_env:
    for p in _ra_env.split(","):
        if p.strip():
            REANCHOR_PRODUCTS.add(p.strip())

_ra_flat_env = os.environ.get("RA_FLAT_PRODUCTS", "").strip()
REANCHOR_FLAT_PRODUCTS = set(part.strip() for part in _ra_flat_env.split(",") if part.strip())
REANCHOR_FLAT_PRODUCTS.add("PEBBLES_L") # v11 default

REANCHOR_EDGE_OVERRIDES: Dict[str, float] = {}
for part in os.environ.get("RA_EDGE_OVERRIDES", "").split(","):
    if ":" in part:
        product, value = part.split(":", 1)
        try:
            REANCHOR_EDGE_OVERRIDES[product.strip()] = float(value)
        except ValueError:
            pass
...
class Trader:
    ...
    def reanchor_orders(self, state: TradingState, memory: Dict[str, Any], product: str) -> List[Order]:
        ...
        deltas = [0.0] * 5 if product in REANCHOR_FLAT_PRODUCTS else spec["deltas"]
        fair = float(anchor_value) + float(deltas[bucket])
        bid, ask = best_bid_ask(depth)
        if bid is None or ask is None:
            return []
        position = int(state.position.get(product, 0))
        target = position
        edge = float(REANCHOR_EDGE_OVERRIDES.get(product, spec["edge"]))
        if ask < fair - edge:
            target = LIMIT
        elif bid > fair + edge:
            target = -LIMIT
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            if product in REANCHOR_PRODUCTS or product in FREE_ALPHA_PRODUCTS or product not in MM_PRODUCTS or product in ACTIVE_RESIDUAL_TARGETS or product in orders:
                continue
            bid, ask = best_bid_ask(depth)
