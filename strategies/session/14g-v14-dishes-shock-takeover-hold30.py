...
# v14 + ROBOT_DISHES shock with ON-DEMAND takeover of re-anchor/reduce-MM.
# Shock fires on |log_ret|>0.5%. When fired, claims DISHES regardless of how
# the position got there (re-anchor or prior shock), flipping to fade
# direction. Holds with passive limit at anchor for up to 20 ticks; on
# time-up or anchor revert flattens at touch and releases. While dormant
# (no shock fired and no position from a prior shock), v14 re-anchor +
# reduce-only MM own DISHES bit-identically to baseline. D2/D3 had zero
# qualifying shocks so they remain bit-identical to v14.
LIMIT = 10
...
class Trader:
    ...
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Tuple[List[Order], bool]:
        ...
        # Returns (orders, claim). claim=True forces re-anchor and reduce-MM
        # to skip DISHES this tick. claim=False lets v14 baseline logic run.
        ...
        out: List[Order] = []
        if shock:
            # Take over unconditionally. Anchor = last_mid before the jump,
            # so a fade target wins when price reverts to last_mid.
            ds["open"] = True
            ...
            return out, True
        if ds.get("open"):
            # Releaser cases: position was externally cleared, or no anchor.
            if pos == 0:
            ...
            ds["last_mid"] = round(float(mid), 4)
            return out, True
        # Dormant: no shock, no shock-owned position. v14 logic owns DISHES.
        ds["last_mid"] = round(float(mid), 4)
        return [], False
