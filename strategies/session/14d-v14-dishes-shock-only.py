...
# v14 with ROBOT_DISHES owned entirely by signal1's shock sleeve.
# Re-anchor and reduce-MM are removed for DISHES. Shock owns it every tick.
# On D2/D3 with zero qualifying shocks, DISHES never trades (lose v14
# baseline ~+19k). On D4, shock captures full signal1 D4 dishes alpha
# (signal1 standalone D4 dishes = +172k vs v14 +51k).
LIMIT = 10
...
MM_PRODUCTS.update({"PEBBLES_L", "SNACKPACK_CHOCOLATE"})
MM_PRODUCTS.discard("ROBOT_DISHES")
...
MM_OWNED_PRODUCTS = {"PEBBLES_L", "SNACKPACK_CHOCOLATE"}
...
class Trader:
    ...
    def nn_cross_orders(self, state: TradingState, memory: Dict[str, Any], spec: Dict[str, Any], tick: int) -> List[Order]:
        ...
        return [order]
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> List[Order]:
        # signal1 fade-and-flip with passive limit at anchor for DISHES.
        ...
        if bid is None or ask is None or mid is None:
            return []
        ds = memory.setdefault("ds", {"last_mid": float(mid), "anchor": None, "hold": 0})
        ...
        if pos != 0 and not shock:
            ...
            if not isinstance(anchor_v, (int, float)):
                if pos > 0:
                    out.append(Order(product, bid, -abs(pos)))
                else:
                    out.append(Order(product, ask, abs(pos)))
            elif int(ds["hold"]) >= ROBOT_SHOCK_MAX_HOLD:
                if pos > 0:
                    out.append(Order(product, bid, -abs(pos)))
                else:
                    out.append(Order(product, ask, abs(pos)))
    ...
    def run(self, state: TradingState):
        ...
        ds_orders = self.dishes_shock_orders(state, memory, tick)
        if ds_orders:
