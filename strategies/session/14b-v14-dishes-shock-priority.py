...
# v14 + ROBOT_DISHES shock-priority sleeve (signal1 fade-and-flip).
# Shock module manages DISHES whenever it has opened a position (ds.open).
# It enters from flat on a |log_ret|>0.5% jump (size 10, fade direction),
# flips to opposite side on subsequent opposite shocks, and flattens on
# 20-tick max hold or when mid reverts to entry anchor. ds.open=True
# claims DISHES for the tick, suppressing v14 re-anchor and reduce-only MM.
# When shock is dormant (no shocks ever or fully closed), v14 logic owns
# DISHES untouched. D2/D3 had zero qualifying shocks so behavior is bit
# identical to v14 baseline on those days.
LIMIT = 10
ROBOT_SHOCK_THRESHOLD = 0.005
ROBOT_SHOCK_MAX_HOLD = 20
ROBOT_SHOCK_ENTRY_SIZE = 10
ROBOT_SHOCK_PRODUCT = "ROBOT_DISHES"
...
MM_PRODUCTS.add("TRANSLATOR_SPACE_GRAY")
MM_SIZE_BY_PRODUCT = {
...
class Trader:
    ...
    def nn_cross_orders(self, state: TradingState, memory: Dict[str, Any], spec: Dict[str, Any], tick: int) -> List[Order]:
        ...
        return [order]
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Tuple[List[Order], bool]:
        # signal1 fade-and-flip shock for ROBOT_DISHES, gated by ds["open"]
        # so the flatten path never overrides v14 re-anchor/reduce-MM unless
        # this module itself opened the position. Returns (orders, claim).
        product = ROBOT_SHOCK_PRODUCT
        depth = state.order_depths.get(product)
        if depth is None:
            return [], False
        bid, ask = best_bid_ask(depth)
        ...
        if bid is None or ask is None or mid is None:
            return [], False
        ds = memory.setdefault("ds", {"last_mid": float(mid), "open": False, "anchor": None, "entry_tick": None})
        last_mid = float(ds.get("last_mid", float(mid)))
        log_ret = math.log(float(mid) / last_mid) if last_mid > 0 else 0.0
        shock = abs(log_ret) > ROBOT_SHOCK_THRESHOLD
        pos = int(state.position.get(product, 0))
        out: List[Order] = []
        if ds.get("open"):
            # External fill (e.g. residual targets that reference DISHES) cleared us.
            if pos == 0 and not shock:
                ds["open"] = False; ds["anchor"] = None; ds["entry_tick"] = None
                ds["last_mid"] = round(float(mid), 4)
                return [], False
            if shock:
                # Reset anchor + hold on every fresh shock; flip to fade direction.
                ds["anchor"] = float(last_mid)
                ds["entry_tick"] = int(tick)
                if log_ret > 0:
                ...
                if delta < 0:
                    out.append(Order(product, bid, delta))
                elif delta > 0:
                    out.append(Order(product, ask, delta))
                ds["last_mid"] = round(float(mid), 4)
                return out, True
            # No shock this tick, still own a position. Try to flatten on time-up
            # or anchor revert; otherwise hold and claim the product.
            entry_tick = int(ds.get("entry_tick", tick))
            anchor_v = ds.get("anchor")
            time_up = (tick - entry_tick) >= ROBOT_SHOCK_MAX_HOLD
            reverted = isinstance(anchor_v, (int, float)) and (
                (pos > 0 and float(mid) >= float(anchor_v)) or
                (pos < 0 and float(mid) <= float(anchor_v))
            )
            if time_up or reverted or not isinstance(anchor_v, (int, float)):
                if pos > 0:
                    out.append(Order(product, bid, -abs(pos)))
                elif pos < 0:
                    out.append(Order(product, ask, abs(pos)))
                ds["open"] = False; ds["anchor"] = None; ds["entry_tick"] = None
                ds["last_mid"] = round(float(mid), 4)
                return out, True
            ds["last_mid"] = round(float(mid), 4)
            return [], True
        # ds.open == False: only enter from a true flat-and-shock state. If
        # re-anchor already owns DISHES (pos != 0 from prior tick), don't take
        # over; let re-anchor manage. Once it flattens, we can engage on the
        # next shock.
        if shock and pos == 0:
            ds["open"] = True
            ds["anchor"] = float(last_mid)
            ds["entry_tick"] = int(tick)
            if log_ret > 0:
                out.append(Order(product, bid, -ROBOT_SHOCK_ENTRY_SIZE))
            else:
                out.append(Order(product, ask, ROBOT_SHOCK_ENTRY_SIZE))
            ds["last_mid"] = round(float(mid), 4)
            return out, True
        ds["last_mid"] = round(float(mid), 4)
        return [], False
    def run(self, state: TradingState):
        ...
        ds_orders, ds_claim = self.dishes_shock_orders(state, memory, tick)
        if ds_orders:
            orders[ROBOT_SHOCK_PRODUCT] = ds_orders
        for product in REANCHOR_PRODUCTS:
            if ds_claim and product == ROBOT_SHOCK_PRODUCT:
                continue
