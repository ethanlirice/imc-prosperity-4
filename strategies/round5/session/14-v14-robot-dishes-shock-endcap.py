...
# v14 + ported alt-strategies/signal1.py ROBOT_DISHES shock sleeve (end-of-run override).
LIMIT = 10
...
class Trader:
        ...
        if trader_data:
            try:
                ...
                if isinstance(data, dict):
                    data.setdefault("tick", 0); data.setdefault("res", {}); data.setdefault("mr", {}); data.setdefault("ra", {}); data.setdefault("rs", {}); return data
            except Exception:
                pass
        return {"tick": 0, "res": {}, "mr": {}, "ra": {}, "rs": {}}
    ...
    def nn_cross_orders(self, state: TradingState, memory: Dict[str, Any], spec: Dict[str, Any], tick: int) -> List[Order]:
        ...
        return [order]
    def robot_dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any]) -> List[Order]:
        ...
        if mid is None:
            return []
        shock_mem = memory.setdefault("rs", {}).setdefault(product, {"last_mid": mid, "anchor": None, "hold": 0})
        last_mid = float(shock_mem.get("last_mid", mid))
        ...
        if pos != 0 and not shock:
            hold = int(shock_mem.get("hold", 0)) + 1
            shock_mem["hold"] = hold
            anchor = shock_mem.get("anchor")
            if not isinstance(anchor, (int, float)):
                ...
                if pos > 0:
                    px = max(int(round(float(anchor))), bid + 1)
                    out.append(Order(product, px, -abs(pos)))
                else:
                    px = min(int(round(float(anchor))), ask - 1)
                    out.append(Order(product, px, abs(pos)))

        if shock:
            shock_mem["anchor"] = last_mid
            shock_mem["hold"] = 0
            if log_ret > 0:
        ...
        if pos == 0 and not shock:
            shock_mem["anchor"] = None
            shock_mem["hold"] = 0
        shock_mem["last_mid"] = round(mid, 4)
        return out
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product_orders:
                orders[product] = product_orders
        shock_o = self.robot_dishes_shock_orders(state, memory)
        if shock_o:
            orders["ROBOT_DISHES"] = shock_o
