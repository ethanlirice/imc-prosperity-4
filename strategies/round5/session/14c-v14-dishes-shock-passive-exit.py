...
# v14 + ROBOT_DISHES shock with signal1's passive-limit exit at anchor.
# While shock holds inventory and no fresh shock fires, posts a passive
# limit at the entry anchor (or one tick inside the spread, whichever is
# better) to capture price reverting to anchor without paying the spread.
# Time-up flatten at touch after 20 ticks. ds.open=True suppresses re-anchor
# and reduce-only MM for DISHES. D2/D3 have zero qualifying shocks so v14
# logic owns DISHES bit-identically on those days.
LIMIT = 10
...
class Trader:
    ...
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Tuple[List[Order], bool]:
        # signal1 fade-and-flip with passive-limit exit at anchor for DISHES.
        ...
        if bid is None or ask is None or mid is None:
            return [], False
        ds = memory.setdefault("ds", {"last_mid": float(mid), "open": False, "anchor": None, "entry_tick": None, "hold": 0})
        ...
        out: List[Order] = []
        if ds.get("open"):
            # Passive exit filled (position cleared) and no fresh shock: release.
            if pos == 0 and not shock:
                ds["open"] = False; ds["anchor"] = None; ds["entry_tick"] = None; ds["hold"] = 0
                ds["last_mid"] = round(float(mid), 4)
                return [], False
            if shock:
                # Fresh shock: flip to fade direction at touch, reset anchor/hold.
                ...
                ds["last_mid"] = round(float(mid), 4)
                return out, True
            # No fresh shock, pos != 0. Either flatten at touch (time-up / no anchor)
            # or post a passive limit at anchor (one tick inside the spread max).
            ...
            if pos > 0:
                # long → sell at max(anchor, bid+1) so we wait for revert up
                px = max(anchor_int, int(bid) + 1)
                out.append(Order(product, px, -abs(pos)))
            elif pos < 0:
                # short → buy at min(anchor, ask-1) so we wait for revert down
                px = min(anchor_int, int(ask) - 1)
                out.append(Order(product, px, abs(pos)))
            ds["last_mid"] = round(float(mid), 4)
            return out, True
        # ds.open == False: open only from flat-and-shock.
        if shock and pos == 0:
            ds["open"] = True
            ds["anchor"] = float(last_mid)
            ds["entry_tick"] = int(tick)
