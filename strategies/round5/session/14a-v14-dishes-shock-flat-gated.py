...
# v14 + strict ROBOT_DISHES shock-only-when-flat sleeve.
# Shock fires only when DISHES inventory == 0 and re-anchor hasn't claimed
# the product this tick. While shock holds inventory, suppresses re-anchor
# and reduce-only MM for DISHES until shock closes.
LIMIT = 10
...
class Trader:
    ...
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Tuple[List[Order], bool]:
        # Returns (orders, claim_dishes). claim_dishes=True forces re-anchor and
        # MM to skip ROBOT_DISHES this tick because shock owns the product.
        ...
        if bid is None or ask is None or mid is None:
            return [], False
        ds = memory.setdefault("ds", {"last_mid": float(mid), "open": False, "anchor": None, "entry_tick": None, "side": 0})
        ...
        out: List[Order] = []
        if ds.get("open"):
            # If actual position drifted to 0 (e.g. external fill), reset and release.
            if pos == 0:
                ds["open"] = False; ds["anchor"] = None; ds["entry_tick"] = None; ds["side"] = 0
            ...
            if time_up or reverted or not isinstance(anchor_v, (int, float)):
                ...
                return out, True
            ds["last_mid"] = round(float(mid), 4)
            return [], True  # hold; suppress re-anchor/MM for DISHES
        # ds.open == False: consider opening on shock if flat
        if shock and pos == 0:
            ds["open"] = True
