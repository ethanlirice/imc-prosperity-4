...
# 15a-translator-pairs.py
# Extraction of T42 Translator cluster from signal2.py into v14g baseline.
# Targets: TRANSLATOR_ASTRO_BLACK (+15.6k), TRANSLATOR_GRAPHITE_MIST (+8.3k).
# Changes:
# 1. Added pair_trading_orders sleeve.
# 2. Enabled MM for ASTRO_BLACK (Bid-OFF removed).
# 3. Added 5 T42 Translator pairs.

LIMIT = 10
ROBOT_SHOCK_THRESHOLD = 0.005
ROBOT_SHOCK_MAX_HOLD = 30
ROBOT_SHOCK_ENTRY_SIZE = 10
ROBOT_SHOCK_PRODUCT = "ROBOT_DISHES"

# T42 TRANSLATOR PAIRS
TRANSLATOR_PAIRS = [
...
# Removed TRANSLATOR_ASTRO_BLACK, TRANSLATOR_SPACE_GRAY from MM_BID_OFF_PRODUCTS for extraction.
...
class Trader:
    def load_memory(self, trader_data: str) -> Dict[str, Any]:
        if trader_data:
            try:
                ...
                pass
        return {"tick": 0, "res": {}, "mr": {}, "ra": {}, "stats": {}, "target": {}}
    ...
    def nn_cross_orders(self, state: TradingState, memory: Dict[str, Any], spec: Dict[str, Any], tick: int) -> List[Order]:
        ...
        return [order]
    def pair_trading_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Dict[str, List[Order]]:
        stats = memory.setdefault("stats", {})
        targets = memory.setdefault("target", {})
        leg_targets = {}
        for a, b, sign, entry_z in TRANSLATOR_PAIRS:
            da, db = state.order_depths.get(a), state.order_depths.get(b)
            if da is None or db is None: continue
            ma, mb = mid_price(da), mid_price(db)
            ...
            if cur == 0:
                ...
                if abs(z) < TRANSLATOR_EXIT_Z: cur = 0
            targets[key] = cur
            ta = 10 * cur
            tb = (-10 if sign == "spread" else 10) * cur
        ...
        for p, tgt in leg_targets.items():
            ...
            result = active_price_for_qty(depth, side, abs(d))
            if result:
                price, qty = result
                out[p] = [Order(p, price, qty if side > 0 else -qty)]
        return out
    def dishes_shock_orders(self, state: TradingState, memory: Dict[str, Any], tick: int) -> Tuple[List[Order], bool]:
        ...
        if bid is None or ask is None or mid is None:
            return [], False
        ds = memory.setdefault("ds", {"last_mid": float(mid), "open": False, "anchor": None, "hold": 0})
        ...
        if shock:
            ...
            ds["hold"] = 0
            target_pos = -ROBOT_SHOCK_ENTRY_SIZE if log_ret > 0 else ROBOT_SHOCK_ENTRY_SIZE
        ...
        if ds.get("open"):
            if pos == 0:
                ds["open"] = False; ds["anchor"] = None; ds["hold"] = 0
                ds["last_mid"] = round(float(mid), 4)
                return [], False
            ds["hold"] = int(ds.get("hold", 0)) + 1
            anchor_v = ds.get("anchor")
            time_up = int(ds["hold"]) >= ROBOT_SHOCK_MAX_HOLD
            if not isinstance(anchor_v, (int, float)) or time_up:
                if pos > 0:
                ...
                ds["last_mid"] = round(float(mid), 4)
                return out, True
            anchor_int = int(round(float(anchor_v)))
            if pos > 0:
                px = max(anchor_int, int(bid) + 1)
                out.append(Order(product, px, -abs(pos)))
            elif pos < 0:
                px = min(anchor_int, int(ask) - 1)
                out.append(Order(product, px, abs(pos)))
            ds["last_mid"] = round(float(mid), 4)
            return out, True
        ds["last_mid"] = round(float(mid), 4)
    ...
    def run(self, state: TradingState):
        ...
        # New Pair Sleeve
        p_orders = self.pair_trading_orders(state, memory, tick)
        for p, ords in p_orders.items():
            if p not in orders: orders[p] = ords

        for product in REANCHOR_PRODUCTS:
            if product in orders or (ds_claim and product == ROBOT_SHOCK_PRODUCT):
                continue
            product_orders = self.reanchor_orders(state, memory, product)
            if product_orders:
                orders[product] = product_orders
        for product in FREE_ALPHA_PRODUCTS:
            if product in REANCHOR_PRODUCTS or product in orders:
                continue
        ...
        for spec in SPECS:
            target = str(spec["target"])
            if target in PASSIVE_TARGET_OVERRIDES or target in FREE_ALPHA_PRODUCTS or target in REANCHOR_PRODUCTS or target in orders:
                continue
