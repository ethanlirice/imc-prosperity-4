...
# Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +1,323,819
# Days: D2 +444,338 / D3 +384,488 / D4 +494,994
# Candidate v10: v8 plus re-anchored hidden-path overlays for ROBOT_DISHES,
# OXYGEN_SHAKE_MINT, SNACKPACK_STRAWBERRY, and SNACKPACK_CHOCOLATE.
LIMIT = 10
...
TLS_SPECS = {
    "oval_triangle": {"target": "MICROCHIP_OVAL", "anchor": "MICROCHIP_TRIANGLE", "method": "ols", "window": 1000, "entry_z": 3.0},
    "triangle_square": {"target": "MICROCHIP_TRIANGLE", "anchor": "MICROCHIP_SQUARE", "method": "ols", "window": 1000, "entry_z": 2.5},
    "laundry_ironing": {"target": "ROBOT_LAUNDRY", "anchor": "ROBOT_IRONING", "method": "ols", "window": 1000, "entry_z": 2.5},
    "strawberry_chocolate": {"target": "SNACKPACK_STRAWBERRY", "anchor": "SNACKPACK_CHOCOLATE", "method": "ols", "window": 2000, "entry_z": 2.5},
    "dishes_laundry": {"target": "ROBOT_DISHES", "anchor": "ROBOT_LAUNDRY", "method": "ols", "window": 1000, "entry_z": 3.0},
    "nylon_lamb": {"target": "SLEEP_POD_NYLON", "anchor": "SLEEP_POD_LAMB_WOOL", "method": "ols", "window": 1000, "entry_z": 3.0},
    "vacuum_mopping_tls": {"target": "ROBOT_VACUUMING", "anchor": "ROBOT_MOPPING", "method": "tls", "window": 1000, "entry_z": 2.5},
    "circle_square_tls": {"target": "MICROCHIP_CIRCLE", "anchor": "MICROCHIP_SQUARE", "method": "tls", "window": 1000, "entry_z": 2.5},
}
_tls_env = os.environ.get("TLS_CASES", "").strip()
TLS_CASES = set(part.strip() for part in _tls_env.split(",") if part.strip())
TLS_PRODUCTS = set(str(TLS_SPECS[name]["target"]) for name in TLS_CASES if name in TLS_SPECS)
TLS_EXIT_Z = float(os.environ.get("TLS_EXIT_Z", "0.25"))
TLS_HOLD = int(os.environ.get("TLS_HOLD", "500"))
TLS_MODE = os.environ.get("TLS_MODE", "active")
TLS_PASSIVE_SIZE = int(os.environ.get("TLS_PASSIVE_SIZE", "1"))
MM_BID_OFF_PRODUCTS = {
    "MICROCHIP_OVAL",
    "MICROCHIP_TRIANGLE",
    "ROBOT_IRONING",
...
class Trader:
        ...
        if trader_data:
            try:
                ...
                if isinstance(data, dict):
                    data.setdefault("tick", 0); data.setdefault("res", {}); data.setdefault("mr", {}); data.setdefault("ra", {}); data.setdefault("tls", {}); return data
            except Exception:
                pass
        return {"tick": 0, "res": {}, "mr": {}, "ra": {}, "tls": {}}
    ...
    def reanchor_orders(self, state: TradingState, memory: Dict[str, Any], product: str) -> List[Order]:
        ...
        price, qty = result
        return [Order(product, price, qty if side > 0 else -qty)]
    def tls_orders(self, state: TradingState, memory: Dict[str, Any], name: str, tick: int) -> List[Order]:
        spec = TLS_SPECS.get(name)
        if spec is None:
            return []
        target = str(spec["target"])
        ...
        if target_depth is None or anchor_depth is None:
            return []
        y_mid = mid_price(target_depth)
        x_mid = mid_price(anchor_depth)
        if y_mid is None or x_mid is None:
            return []
        memory_for_case = memory.setdefault("tls", {}).setdefault(name, {})
        xs = memory_for_case.get("x", [])
        ys = memory_for_case.get("y", [])
        if not isinstance(xs, list): xs = []
        if not isinstance(ys, list): ys = []
        window = int(spec["window"])
        hx = [float(value) for value in xs[-window:] if isinstance(value, (int, float))]
        hy = [float(value) for value in ys[-window:] if isinstance(value, (int, float))]
        orders: List[Order] = []
        if len(hx) >= max(50, window // 2) and len(hy) == len(hx):
            n = float(len(hx))
            mx = sum(hx) / n
            my = sum(hy) / n
            vx = sum((value - mx) * (value - mx) for value in hx) / n
            vy = sum((value - my) * (value - my) for value in hy) / n
            cxy = sum((hx[idx] - mx) * (hy[idx] - my) for idx in range(len(hx))) / n
            beta = None
            if str(spec["method"]) == "tls":
                if abs(cxy) > 1e-9:
                    root = ((vy - vx) * (vy - vx) + 4.0 * cxy * cxy) ** 0.5
                    beta = (vy - vx + root) / (2.0 * cxy)
            elif vx > 1e-9:
                beta = cxy / vx
            if beta is not None:
                alpha = my - beta * mx
                sigma = max(0.0, vy + beta * beta * vx - 2.0 * beta * cxy) ** 0.5
                if sigma > 1e-9:
                    z = (float(y_mid) - (alpha + beta * float(x_mid))) / sigma
                    position = int(state.position.get(target, 0))
                    if TLS_MODE == "passive":
                        bid, ask = best_bid_ask(target_depth)
                        if bid is not None and ask is not None and bid < ask:
                            buy_qty = min(LIMIT - position, TLS_PASSIVE_SIZE)
                            sell_qty = min(LIMIT + position, TLS_PASSIVE_SIZE)
                            if z <= -float(spec["entry_z"]) and buy_qty > 0:
                                orders.append(Order(target, bid, buy_qty))
                            elif z >= float(spec["entry_z"]) and sell_qty > 0:
                                orders.append(Order(target, ask, -sell_qty))
                            else:
                                if buy_qty > 0 and target not in MM_BID_OFF_PRODUCTS:
                                    orders.append(Order(target, bid, buy_qty))
                                if sell_qty > 0 and target not in MM_ASK_OFF_PRODUCTS:
                                    orders.append(Order(target, ask, -sell_qty))
                    elif position == 0:
                        memory_for_case.pop("entry_tick", None)
                        direction = 1 if z <= -float(spec["entry_z"]) else -1 if z >= float(spec["entry_z"]) else 0
                        if direction != 0:
                            order = self.entry_order(state, target, direction)
                            if order is not None and order.quantity != 0:
                                orders.append(order)
                                memory_for_case["entry_tick"] = tick
                    else:
                        entry_tick = int(memory_for_case.get("entry_tick", tick))
                        zero_cross = (position > 0 and z >= -TLS_EXIT_Z) or (position < 0 and z <= TLS_EXIT_Z)
                        if zero_cross or tick - entry_tick >= TLS_HOLD:
                            order = self.close_order(state, target, position)
                            if order is not None:
                                orders.append(order)
                                memory_for_case.pop("entry_tick", None)
        hx.append(float(x_mid))
        hy.append(float(y_mid))
        memory_for_case["x"] = hx[-window:]
        memory_for_case["y"] = hy[-window:]
        return orders
    def run(self, state: TradingState):
        ...
        for name in TLS_CASES:
            product_orders = self.tls_orders(state, memory, name, tick)
            if product_orders:
                orders[product_orders[0].symbol] = product_orders
        for product in REANCHOR_PRODUCTS:
            if product in TLS_PRODUCTS:
                continue
            product_orders = self.reanchor_orders(state, memory, product)
            if product_orders:
                orders[product] = product_orders
        for product in FREE_ALPHA_PRODUCTS:
            if product in TLS_PRODUCTS or product in REANCHOR_PRODUCTS:
                continue
        ...
        for spec in SPECS:
            target = str(spec["target"])
            if target in TLS_PRODUCTS or target in PASSIVE_TARGET_OVERRIDES or target in FREE_ALPHA_PRODUCTS or target in REANCHOR_PRODUCTS:
                continue
        ...
        for product, depth in state.order_depths.items():
            if product in TLS_PRODUCTS or product in REANCHOR_PRODUCTS or product in FREE_ALPHA_PRODUCTS or product not in MM_PRODUCTS or product in ACTIVE_RESIDUAL_TARGETS or product in orders:
                continue
            bid, ask = best_bid_ask(depth)
