"""
Round 5 trader v2 — MM + MR only.

Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +152,042.

v1 ran a three-layer stack and hit +79,513. Per-layer attribution showed:
  MM:   +103,310
  MR:   +10,286
  stat: -34,083  (every confirmed pair lost — spread-crossing cost on each
                  entry/exit dwarfed the +2k/pair edge predicted from mid-mid
                  walk-forward Sharpe)

v2 deletes Layer 3, frees those products to be MM/MR-traded, keeps the same
sticky-fair EWMA + 1-tick passive quote architecture.
"""
...
# v2: stat arb disabled (every pair lost in live sim due to spread-crossing
# cost). Keep PAIRS empty; the existing Layer 3 functions become no-ops.
