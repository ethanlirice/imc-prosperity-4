...
# Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +822,226
# Days: D2 +287,132 / D3 +212,359 / D4 +322,734
# Experiment: current touch-MM candidate, but stable products from the supplied
# five-bucket "free alpha" path own their product instead of residual/MM.
...
MM_PRODUCTS = MM_PRODUCTS - FREE_ALPHA_PRODUCTS
...
def free_alpha_orders(state: TradingState, product: str) -> List[Order]:
    ...
    target = position
    if ask < fair - FREE_ALPHA_ENTRY_EDGE:
        target = LIMIT
    elif bid > fair + FREE_ALPHA_ENTRY_EDGE:
        target = -LIMIT
    else:
        mid = 0.5 * (bid + ask)
