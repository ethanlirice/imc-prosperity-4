"""
Round 5 trader v1 — three-layer architecture.

Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +79,513.

Layer 1 (passive market making):    all 50 products, quotes inside best bid/ask.
Layer 2 (mean reversion takes):     all 50 products, crosses when book is far from EWMA fair.
Layer 3 (statistical arbitrage):    confirmed pairs (Phase 1 walk-forward Sharpe>1).

Position budgeting: stat-arb products are owned exclusively by Layer 3 to avoid
...
Phase 1 survivor pairs from the historical three-layer prototype:
...
# Stat arb pairs that survived walk-forward Sharpe > 1.0 with >= 4 trades.
# Each entry trades spread = mid(a) - mid(b). Long the spread = long a, short b.
PAIRS: List[Dict[str, str]] = [
    {"a": "SNACKPACK_CHOCOLATE", "b": "SNACKPACK_PISTACHIO"},
    {"a": "OXYGEN_SHAKE_CHOCOLATE", "b": "OXYGEN_SHAKE_GARLIC"},
    {"a": "SNACKPACK_PISTACHIO", "b": "SNACKPACK_RASPBERRY"},
]
STAT_ARB_PRODUCTS = {p for pair in PAIRS for p in (pair["a"], pair["b"])}

# PISTACHIO appears in two pairs, so cap each pair's leg size at half the limit
# so two simultaneous opposite signals never breach the position limit.
PAIR_SIZE = 5

# Spread z-score thresholds and rolling window. Window picked from the brief
# (Layer 3 spec). Warmup avoids firing on noise.
WINDOW = 500
MIN_WARMUP = 200
ENTRY_Z = 1.5
EXIT_Z = 0.5
