# ============================================================================
# multistrat.py — v314159 + VFE-only EWMA / roll-bias overlay (exploration)
# ============================================================================
# Layers from strategies/round4/alternative.py that helped VFE in isolation:
#   EWMA(mid), z_local = (mid−rolling_mean)/static_sd,
#   drift_z = (rolling_mean−static_mean)/static_sd,
#   z = z_local + k*drift_z (k=0.95). When |z| exceeds thresh, tilt VFE passive
#   and active edges vs baseline OU stack (everything else unchanged).
#
# Tunables: MULTISTRAT_VFE_* below. Disable overlay by setting Z_THRESH very high.
# ============================================================================
...
# --- multistrat: VFE z overlay (aligned with alternative.py VFE CFG) ---
MULTISTRAT_VFE_Z_ALPHA = 0.0025
MULTISTRAT_VFE_STATIC_MEAN = 5247.0
MULTISTRAT_VFE_STATIC_SD = 17.091
MULTISTRAT_VFE_K_ROLL = 0.95
MULTISTRAT_VFE_Z_THRESH = 1.0


def _multistrat_update_vfe_z(mid, ss):
    """Rolling EWMA + composite z; mutates ss with _ms_vfe_* keys."""
    sm = MULTISTRAT_VFE_STATIC_MEAN
    sd = MULTISTRAT_VFE_STATIC_SD
    alpha = MULTISTRAT_VFE_Z_ALPHA
    k_roll = MULTISTRAT_VFE_K_ROLL
    mp = ss.get("_ms_vfe_m", sm)
    vp = ss.get("_ms_vfe_v", sd * sd)
    ss["_ms_vfe_n"] = ss.get("_ms_vfe_n", 0) + 1
    dev = mid - mp
    rm = (1.0 - alpha) * mp + alpha * mid
    rv = (1.0 - alpha) * vp + alpha * dev * dev
    ss["_ms_vfe_m"] = rm
    ss["_ms_vfe_v"] = rv
    if rv <= 0 or sd <= 0:
        return 0.0
    z_loc = (mid - rm) / sd
    drift_z = (rm - sm) / sd
    return z_loc + k_roll * drift_z


def _multistrat_vfe_overlay_edges(z):
    """Return (bid_pe, ask_pe, take_buy_e, take_sell_e) or None = use v314159 defaults."""
    thresh = MULTISTRAT_VFE_Z_THRESH
    if abs(z) < thresh:
        return None
    scale = min(2.0, abs(z) - thresh)
    bb = PASSIVE_VFE_BID_EDGE
    ba = PASSIVE_VFE_ASK_EDGE
    ue = UNDERLYING_EDGE
    if z > 0.0:
        bid_pe = max(6.0, bb - scale)
        ask_pe = max(3.0, ba - 0.5 * scale)
        take_buy = min(14.0, ue + 0.5 * scale)
        take_sell = max(8.0, ue - scale)
    else:
        bid_pe = max(6.0, bb - 0.5 * scale)
        ask_pe = max(3.0, ba - scale)
        take_buy = max(8.0, ue - scale)
        take_sell = min(14.0, ue + 0.5 * scale)
    return bid_pe, ask_pe, take_buy, take_sell
...
class Trader:
    ...
    def run(self, state):
        ...
        bid_pe = PASSIVE_VFE_BID_EDGE
        ask_pe = PASSIVE_VFE_ASK_EDGE
        take_buy_e = None
        take_sell_e = None
        if vvf_spot is not None:
            z_sig = _multistrat_update_vfe_z(float(vvf_spot), ss)
            ov = _multistrat_vfe_overlay_edges(z_sig)
            if ov is not None:
                bid_pe, ask_pe, take_buy_e, take_sell_e = ov

        # --- VVF (M67 sells-suppression + multistrat VFE z overlay) ---
        if UNDERLYING in state.order_depths:
            ...
            orders = self.trade_ou_split(
                ...
                take_buy_edge=take_buy_e,
                take_sell_edge=take_sell_e,
            )
            if orders:
