# ============================================================================
# big_swing.py — bold but minimal: debounced Mark 14 + width lift + M67 flash ONLY
# ============================================================================
# Exactly three structural moves vs v314159.py:
#   (1) Empirical width co-tuning (HIGH_GAMMA_EDGE / UNDERLYING_EDGE, v322221-class).
#   (2) Join-bid flash for MERGED_VFE_FLASH_MS after M67/M49 (merged.py pattern).
#   (3) No ITM clips, beta, Mark22, or PASSIVE_SIZE experiments — avoids negative
#       interference seen when stacking junk knobs onto Mark layers.
#
# Local R4 merged D1–D3 backtest vs strategies/round4/v314159.py baseline:
#   none: ~956.9k vs ~950.8k  (~+6.1k); worse: ~963.3k vs ~957.5k (~+5.8k).
# ============================================================================
...
# Edges
UNDERLYING_EDGE = 12.6
HYDROGEL_EDGE = 12.0
OPTION_EDGE = 9.5
HIGH_GAMMA_EDGE = 1.65
...
# Mark 14 — debounced HGP buy pulse (cooldown between accepted pulses)
M14_BUYER = "Mark 14"
M14_DEBOUNCE_GAP_TS = 5000
M14_DEBOUNCE_WINDOW = SIGNAL_WINDOW

MERGED_VFE_FLASH_MS = 400
...
class Trader:
    ...
    def update_m14_debounce_hgp(self, state, ss):
        """Cooldown debounce on Mark 14 HGP buys — one pulse per DEBOUNCE_GAP."""
        last_acc = ss.get("m14_db_last_acc_ts", -10**9)
        pulse_ts = ss.get("m14_db_pulse_ts", -10**9)
        trades = state.market_trades.get(HYDROGEL, [])
        for tr in sorted(trades, key=lambda x: x.timestamp):
    ...
    def run(self, state):
        ...
        ss = self.update_m14_debounce_hgp(state, ss)
        ...
        m14_pulse = ss.get("m14_db_pulse_ts", -10**9)
        m14_suppress_hgp = (
            (state.timestamp - m14_pulse) <= M14_DEBOUNCE_WINDOW and m14_pulse >= 0
        )

        vfe_bid_pe = PASSIVE_VFE_BID_EDGE
        if signal_active:
            dt_f = state.timestamp - last_ts
            if dt_f <= MERGED_VFE_FLASH_MS:
                vfe_bid_pe = MERGED_VFE_FLASH_BID_EDGE

        # --- VVF (M67 suppression + flash passive bid) ---
        ...
        if HYDROGEL in state.order_depths:
            ...
            orders = self.trade_ou_split(
                ...
                suppress_sell=m14_suppress_hgp,
            )
            if orders:
