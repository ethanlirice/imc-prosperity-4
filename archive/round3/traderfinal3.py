"""
v26_no_fv — v25 with ZERO hardcoded fair values.

Changes vs v25 (the four places where FV was fixed):
  L32: VFE_ANCHOR_FOR_OPTIONS=5250  -> derived from online VFE anchor
  L70: VFE_ANCHOR=5250              -> rolling median of wm (W=2000, warmup=200)
  L108: vfe_ema seed=5250.0         -> seed from current online anchor
  L145: HGP FAIR=9985               -> rolling median of wm (W=2000, warmup=200)

Kept from v25 (these are meta-parameters, not fixed fair values):
  SCALE, TAKE_EDGE, TAKE_CLIP, edge_floors, alphas, betas, smile sleeve.

Empirical cost (measured on R3 days 0-2, VFE only):
  v25 static (5250 hardcoded) + cap=15:  +98,963
  v26 rolling median W=2000 + cap=15:    +89,846  (-9,117  / -9.2%)

Benefit:
  - Portable to Round 4 without recalibration
  - Does not depend on knowing simulator equilibrium
  - Tracks mean shifts across rounds

Estimator details:
  - Median over wall_mid for the last 2000 ticks (about 10x half-life)
  - Warmup: 200 ticks with no trading (avoid initial path bias in anchor)
  - Median instead of mean: more robust to outliers; for symmetric OU,
    median = mean = mu, but median converges better when path coverage
    is uneven across both sides
  - For deep ITM options (approx S - K): reuse online VFE anchor
    instead of hardcoding 5250 - K
"""
...
# Online-anchor hyperparameters (NOT prices, these are estimator settings)
...
class Trader:
    ...
    def online_anchor(self, key, sample):
        """Update rolling history and return current median, or None during warmup.
        State is stored in self.sd[f'{key}_hist'] as a list of floats."""
    ...
    def vfe_scaled(self, depth, pos):
        """v25 chassis with VFE_ANCHOR replaced by an online rolling median."""
        ...
        if anchor_f is None:
            # During warmup we still record data but do not trade
            return []
        VFE_ANCHOR = int(round(anchor_f))
        # Cache for ITM options to reuse (avoids recomputing)
        ...
        # Seed EMA with current online anchor instead of hardcoded 5250.0
        VFE_EMA_ALPHA = 0.005
        VFE_EMA_TE = 14
        mid_for_ema = (b + a) / 2
    ...
    def hgp_scaled(self, depth, pos):
        """v25 chassis with HGP FAIR replaced by an online rolling median."""
        sym = "HYDROGEL_PACK"
        if not depth.buy_orders or not depth.sell_orders:
            return []
        wm = self.wall_mid(depth)
    ...
    def trade_itm_option(
        ...
        """Deep ITM options. FV = E[S] - K, where E[S] is the online VFE anchor.
        Replaces v25 hardcoded ITM_STRUCTURAL_FV = 5250 - K."""
        if not depth.buy_orders or not depth.sell_orders:
            return
    ...
    def run(self, state):
        ...
        # Order matters: run VFE first so vfe_anchor_current is cached for ITM options
