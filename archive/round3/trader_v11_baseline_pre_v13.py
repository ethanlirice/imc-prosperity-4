"""
Round 3 trader — fundamentals + microstructure, with Black-Scholes done correctly.

Core PnL (backtest-proven on historical days 0-2):
- HYDROGEL_PACK: wall-based MM
- VEV_4000: wall-based MM (same microstructure as HYDROGEL, wide spread, stable L2)
- VELVETFRUIT_EXTRACT: simple spread capture (penny inside L1) with inventory skew

Options module (VEV_5300):
- Uses Black-Scholes with an offline IV smile.
- Infers the session's start-TTE (5/6/7/8 days) from the option surface.
- **Only activates when inferred start-TTE is 5 days** (Round 3 live regime).
  Historical days 0-2 infer 8/7/6, so we avoid mis-applying the live clock in backtests.
"""
...
# Option surface used to infer the start-of-session days-left.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        oo = self.trade_vev_5300_bs_live(state, td)
        if oo:
            result[VEV_5300] = oo
    ...
    def infer_start_days(self, state: TradingState):
    ...
    def trade_vev_5300_bs_live(self, state: TradingState, td: dict):
        # Only trade options in the live Round 3 regime (start-TTE = 5 days).
        ...
        warm = td.get("vev5300_warm", 0) + 1
        td["vev5300_warm"] = warm
        if warm < EMA_WARMUP:
            return []
