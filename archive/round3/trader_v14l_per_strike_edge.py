...
class Trader:
    ...
    # ── v14j: EMA wall_mid HGP fair + v14f tight edge in core option sleeve ──
    def hgp_ntrader(self, depth, pos):
        ...
        prev_fair = self.sd.get('hgp_fair')
        if prev_fair is None:
            fair_value = wm if wm is not None else HYDROGEL_FAIR
        else:
            fair_value = prev_fair + 0.005 * ((wm if wm is not None else prev_fair) - prev_fair)
    ...
    def trade_core_option(self, sym, depth, pos, out):
        ...
        # Per-strike edge: deep ITM (4000/4500) needs wider edge (delta-1 noise);
        # NTM (5000-5300) benefits from tighter edge.
        K = int(sym.split('_')[-1])
        if K <= 4500:
            dynamic_edge = max(12.0, spread * 1.5)
        else:
