...
# Primary R5 backtest: prosperity4btx ... 5 --merge-pnl = +1,349,478
# Days: D2 +440,342 / D3 +401,360 / D4 +507,776
# Generated NN cross-pressure research variant: galaxy_black_holes_ret100_high_flames_short_h1000.
LIMIT = 10
...
class Trader:
    ...
    def nn_cross_orders(self, state: TradingState, memory: Dict[str, Any], spec: Dict[str, Any], tick: int) -> List[Order]:
        ...
        if not isinstance(mids, list):
            mids = []
        history = [float(value) for value in mids[-int(spec["lookback"]):] if isinstance(value, (int, float))]
        src_mem["mids"] = (history + [float(source_mid)])[-int(spec["lookback"]):]
        if len(history) < int(spec["lookback"]):
            return []
